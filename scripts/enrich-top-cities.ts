...
/**
 * Prioritized Church Enrichment Script - Top 50 US Cities
 *
 * Enriches churches in major US cities first, then continues with remaining.
 * Generates:
 ...
 *
 * Usage:
 *   npx tsx scripts/enrich-top-cities.ts
 *   npx tsx scripts/enrich-top-cities.ts --workers 5
 */
...
const DELAY_BETWEEN_CHURCHES = 1000;
...
class CityEnrichmentWorker {
  private workerId: number;
  private cities: { city: string; state: string }[];
  ...
  async run(): Promise<WorkerStats> {
    console.log(`[Worker ${this.workerId}] Starting with ${this.cities.length} cities`);

    for (const { city, state } of this.cities) {
      if (!this.running) break;

      this.stats.currentCity = `${city}, ${state}`;
      await this.processCity(city, state);
    }

    console.log(`[Worker ${this.workerId}] ✅ Completed all assigned cities`);
    return this.stats;
  }

  private async processCity(city: string, state: string): Promise<void> {
    console.log(`[Worker ${this.workerId}] Processing ${city}, ${state}...`);

    while (this.running) {
      // Get batch of unenriched churches for this city
      ...
      if (!churches || churches.length === 0) {
        console.log(`[Worker ${this.workerId}] ✓ ${city}, ${state} complete (${this.stats.success} enriched)`);
        break;
      }
      ...
      for (const church of churches) {
        ...
        try {
          ...
          if (enrichment.worshipStyle) updateData.worship_style = enrichment.worshipStyle;
          if (enrichment.serviceTimes) updateData.service_times = enrichment.serviceTimes;
          if (enrichment.hasKidsMinistry !== undefined) updateData.has_kids_ministry = enrichment.hasKidsMinistry;
          if (enrichment.hasYouthGroup !== undefined) updateData.has_youth_group = enrichment.hasYouthGroup;
          if (enrichment.hasSmallGroups !== undefined) updateData.has_small_groups = enrichment.hasSmallGroups;
      ...
      // Log progress
      console.log(
        `[Worker ${this.workerId}] ${city}: ${this.stats.success} enriched, ${this.stats.errors} errors`
      );
    }
  }
...
function distributeCities(cities: { city: string; state: string }[], numWorkers: number): { city: string; state: string }[][] {
  const distribution: { city: string; state: string }[][] = Array.from({ length: numWorkers }, () => []);

  cities.forEach((city, index) => {
    distribution[index % numWorkers].push(city);
  });
...
async function main() {
  ...
  console.log('\n🏙️  Top 50 Cities Church Enrichment\n');
  console.log(`   Workers:        ${numWorkers}`);
  console.log(`   Cities:         ${TOP_50_CITIES.length}`);
  ...
  // Count unenriched in top 50 cities
  let totalUnenriched = 0;
  for (const { city, state } of TOP_50_CITIES) {
    const { count } = await supabase
      ...
      .eq('state', state)
      .is('ai_description', null);
    totalUnenriched += count ?? 0;
  }

  console.log(`📋 Churches to enrich in top 50 cities: ${totalUnenriched.toLocaleString()}\n`);

  // Distribute cities among workers
  const cityDistribution = distributeCities(TOP_50_CITIES, numWorkers);

  // Create workers
  const workers: CityEnrichmentWorker[] = [];

  for (let i = 0; i < numWorkers; i++) {
    const workerSupabase = createClient(supabaseUrl, supabaseServiceKey);
    const worker = new CityEnrichmentWorker(i + 1, cityDistribution[i], workerSupabase, anthropicKey);
    workers.push(worker);

    console.log(`[Worker ${i + 1}] Assigned ${cityDistribution[i].length} cities: ${cityDistribution[i].map(c => c.city).join(', ')}`);
  }
  ...
  for (const { city, state } of TOP_50_CITIES) {
    const { count } = await supabase
      ...
      .eq('state', state)
      .is('ai_description', null);
    remainingInCities += count ?? 0;
  }

  console.log(`📋 Remaining unenriched in top 50 cities: ${remainingInCities.toLocaleString()}\n`);
}
